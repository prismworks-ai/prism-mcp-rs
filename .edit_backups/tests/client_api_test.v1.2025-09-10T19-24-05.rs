...
use prism_mcp_rs::client::{McpClientBuilder, ClientInfo};
...
async fn test_client_builder_connect_stdio() {
    ...
    let _result = builder.connect_stdio(
        "echo",
        &["test".to_string()],
        Some(HashMap::new())
    ).await;
...
async fn test_client_session_methods() {
    use prism_mcp_rs::client::{McpClient, ClientSession};
...
fn test_stdio_transport_with_env() {
    ...
    let _future = StdioClientTransport::with_env(
        "echo",
        vec!["test"],
        env_vars
    );
