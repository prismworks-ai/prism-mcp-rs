# Generated Examples

These examples are automatically generated from the SDK documentation.
**Do not edit these files directly** - edit the source documentation instead.

## Running Examples

```bash
# Run a specific example
cargo run --example <example_name>

# Run with required features
cargo run --example <example_name> --features "feature1 feature2"
```

## Available Examples

### client::mcp_client

- **client_mcp_client_example_1** - Line 1459

### core::completion

- **core_completion_example_1** - Line 43
- **core_completion_example_2** - Line 185
- **core_completion_example_3** - Line 353
- **core_completion_example_4** - Line 447

### core::completion_handlers

- **core_completion_handlers_example_1** - Line 35
- **core_completion_handlers_example_2** - Line 243
- **core_completion_handlers_example_3** - Line 389
- **core_completion_handlers_example_4** - Line 565

### core::tool

- **core_tool_example_1** - Line 399

### server::builder

- **server_builder_example_1** - Line 16
  ///

### server::mcp_server

- **server_mcp_server_example_1** - Line 88
  ///
/// ## Basic Server Creation
- **server_mcp_server_basic_server_creation** - Line 90
- **server_mcp_server_using_serverbuilder** - Line 97

### test_utils::mod

- **test_utils_mod_example_1** - Line 32
- **test_utils_mod_example_2** - Line 45
- **test_utils_mod_example_3** - Line 65
- **test_utils_mod_example_4** - Line 92
- **test_utils_mod_example_5** - Line 111
- **test_utils_mod_example_6** - Line 132
- **test_utils_mod_example_7** - Line 151
- **test_utils_mod_example_8** - Line 172
- **test_utils_mod_example_9** - Line 196
- **test_utils_mod_example_10** - Line 213
- **test_utils_mod_example_11** - Line 225
- **test_utils_mod_example_12** - Line 236
- **test_utils_mod_example_13** - Line 261
- **test_utils_mod_example_14** - Line 276

## Regenerating Examples

To regenerate these examples from the documentation:

```bash
cargo test --test doc_examples
```

Or using the Makefile:

```bash
make examples-generate
```
