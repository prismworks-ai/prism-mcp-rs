...
async fn test_corrected_add_simple_tool_example() {
    ...
    ).await.expect("Failed to add simple tool");
...
async fn test_filesystem_example() {
    ...
    ).await.expect("Failed to add read_file tool");
...
// Test the HTTP tool using full add_tool method
struct HttpTool {
    #[allow(dead_code)] // For testing
    client: String, // Simplified for testing
}
...
async fn test_http_tool_example() {
    ...
    server.add_tool(
        ...
        schema,
        HttpTool { client: "test_client".to_string() }
    ).await.expect("Failed to add HTTP tool");
...
// Test that examples with correct imports compile
#[test]
...
// Test parameter extraction patterns shown in docs
#[test]
fn test_parameter_extraction_patterns() {
    ...
    let flag = args.get("flag")
        .and_then(|v| v.as_bool())
        .unwrap_or(false);
    assert_eq!(flag, true);
}

// Test ContentBlock usage patterns from docs
#[test]
...
// Integration test that exercises the complete flow
#[tokio::test]
async fn test_complete_documentation_flow() {
    ...
    ).await.expect("Failed to add hello tool");
    
    // Test that we can retrieve the tool
    let tools = server.list_tools().await.expect("Failed to list tools");
    let hello_tool = tools.iter().find(|t| t.name == "hello")
        .expect("Hello tool not found");
    
    assert_eq!(hello_tool.name, "hello");
    assert_eq!(hello_tool.description, Some("Says hello to someone".to_string()));
